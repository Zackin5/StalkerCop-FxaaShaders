...
void        hmodel                 (out half3 hdiffuse, out half3 hspecular, half m, half h, half s, float3 point, half3 normal)
{
        ...
        // specular color
                ...
                half vDotN = saturate(dot(normal, v));
                half F0 = light.x * 0.08;
                half roughness = 1 - s;
                half3 F = fresnelSchlickRoughness(vDotN, F0, roughness);
                half2 brdf = EnvironmentBRDF(roughness, vDotN);
                // hspecular = half3(s,s,s);
                // hspecular = env_color.xyz;
                // hspecular = brdf.x + (F * (brdf.y - brdf.x));
                // hspecular = (brdf.x * env_color.xyz) + env_color.xyz * (F * (brdf.y - brdf.x));
                hspecular = (brdf.x * env_s) + env_s * (F * (brdf.y - brdf.x));
        #else
                // vanilla
                                vreflect.y      = vreflect.y*2-1;                                                        // fake remapping
                half3         e0s               = texCUBE         (env_s0,vreflect);
